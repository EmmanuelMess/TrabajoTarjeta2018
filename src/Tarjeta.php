...
class Tarjeta implements TarjetaInterface {
	...
	if($lineacole!=$this->linea || $banderacole != $this->bandera){
		...
		if( $diapago1 == "Lunes" || $diapago1 == "Martes" || $diapago1 == "Miercoles" || $diapago1 == "Jueves" || $diapago1 == "Viernes"){
				if($horapago1 >= "06:00:00" && $horapago1 <= "22:00:00" && $diapago1 == $diapago2 && $diapago1prima == $diapago2prima){echo "DIA";
					$tolerancia += (60 * 60);
				...
				if( $horapago1 > "22:00:00" || $horapago1 < "06:00:00"){echo "NOCHE";
					$tolerancia += (90 * 60);
					$tolerancia = $tiempoayuda->saber_hora($tolerancia);
					echo "TOLERANCIA " ,$tolerancia;
					echo "HORA PAGO 2 ",$horapago2;
		...
		if( $diapago1 == "Sabado"){
				if($horapago1 >= "06:00:00" && $horapago1 <= "14:00:00" && $diapago1 == $diapago2 && $diapago1prima == $diapago2prima){echo "DIA";
					$tolerancia += 60 * 60;
					$tolerancia = $tiempoayuda->saber_hora($tolerancia);
					echo "TOLERANCIA " ,$tolerancia;
					echo "HORA PAGO 2 ",$horapago2;
				...
				if($horapago1 > "14:00:00" && $horapago1 <= "22:00:00" && $diapago1 == $diapago2 && $diapago1prima == $diapago2prima){echo "TARDE";
					$tolerancia += 90 * 60;
					$tolerancia = $tiempoayuda->saber_hora($tolerancia);
					echo "TOLERANCIA " ,$tolerancia;
					echo "HORA PAGO 2 ",$horapago2;
		...
		if( $diapago1 == "Domingo" || in_array($diapagoferiado,$diasferiados)){
				if($horapago1 >= "06:00:00" && $horapago1 <= "22:00:00" && $diapago1 == $diapago2 && $diapago1prima == $diapago2prima){echo "TODO EL DIA";
					$tolerancia += 90 * 60;
					$tolerancia = $tiempoayuda->saber_hora($tolerancia);
					echo "TOLERANCIA " ,$tolerancia;
					echo "HORA PAGO 2 ",$horapago2;
